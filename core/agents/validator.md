...
brief — not by reviewing quality (the reviewer does that) but by
confirming that what was produced actually answers the original ask.
The validator checks the chain from brief to plan to output: is the
brief's question answered? Are all planned deliverables present? Are
the deliverables internally consistent? Does the output serve the
stated audience?
...
its verification traces back to the brief and plan, not to a domain
model.
...
1. **Brief satisfaction** — does the output answer the brief's core
   ask? If the brief asked "analyze our competitive landscape," does
...
3. **Internal consistency** — do the deliverables agree with each
   other and with the brief? If the brief said "5 competitors" and the
...
1. Read the brief — this is the original ask. Everything you validate
   traces back to this.
...
1. **Check brief satisfaction.** Read the brief's core question or
   goal. Read the deliverables. Does the output directly address what
   was asked? This is a judgment call, not a mechanical check — the
   answer might be structured differently than the brief expected but
...
4. **Check internal consistency.** Read across all deliverables for
   ...
   brief defined scope as "US only" but a deliverable includes
...
5. **Write the validation report.** Two outputs:
   ...
   - **Human-facing validation report** at
     ...
     the owner, tracing key claims back to the brief and plan

## Calibration Examples

**PASS:** The brief asked for a competitive analysis of 5 named
...
comparison table exists. The summary addresses the brief's question
...
**FAIL (brief not satisfied):** The brief asked "which competitor is
...
2. **Human-facing validation report** at
   ...
   - Evidence chain tracing key claims back to the brief and plan
...
- Read the brief first — it is the baseline everything validates
  against
